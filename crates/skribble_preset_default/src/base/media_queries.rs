...
use skribble_core::MediaQueries;
use skribble_core::MediaQuery;

use super::DarkMode;

pub fn update_media_queries(media_queries: &mut MediaQueries, dark_mode: DarkMode) {
  media_queries.extend(MEDIA_QUERIES.clone());

  if dark_mode == DarkMode::Media {
    media_queries.extend(DARK_MEDIA_QUERIES.clone());
  }
}

lazy_static! {
  static ref MEDIA_QUERIES: Vec<Group<MediaQuery>> = vec![
    Group::builder()
  ...
  static ref DARK_MEDIA_QUERIES: Vec<Group<MediaQuery>> = vec![
    Group::builder()
      .name("darkMode")
