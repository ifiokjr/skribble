      ...
      Self::Color(color_field) => {
        ...
        if let Some(variable) = config.css_variables.get(name) {
          ...
          let default_value = if transformers.is_empty() {
            ...
            let transformed_color = apply_color_transformers(transformers, config, &mut hsla_css);
            apply_transformers(
              transformed_color,
              transformers,
              config,
        ...
        if let Some(palette_value) = config.palette.get(name) {
          ...
          let default_value = if transformers.is_empty() {
            let transformed_color = options.color_format.get_color(palette_value)?.to_string();
            apply_transformers(
              transformed_color,
              transformers,
              config,
