...
lazy_static! {
	pub(crate) static ref TRANSFORMERS: Vec<Group<Transformer>> = vec![
		Group::builder()
			...
			.items(vec![
				Transformer::builder()
					.name("important")
					.description(
						"Append `!important` to every property in the transformed atom / named \
						 class."
					)
		...
		Group::builder()
			.name("alpha")
			.description(
				"Set color alpha. For custom values the value should be between 0 and 1. Don't \
				 use percentages."
			)
			.items(vec![
				Transformer::builder()
					.name("alpha")
					.description(
						"Sets the alpha value of a color to this value. Should be a value between \
						 0 and 1."
					)
